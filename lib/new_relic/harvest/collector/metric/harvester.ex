defmodule NewRelic.Harvest.Collector.Metric.Harvester do
  ...
  @moduledoc """
    Individual Harvester process. Collects metrics for a single harvest cycle.
  """
  ...
  def aggregate({_ident, metrics}), do: Metric.reduce(metrics)

  def encode(%Metric{name: name, scope: scope} = m),
    do: [
