defmodule NewRelic.Harvest.Collector.HarvestCycle do
  ...
  def init(name: name, module: module, supervisor: supervisor) do
    if NewRelic.Config.enabled?(), do: send(self(), :harvest_cycle)
    {:ok, %{name: name, module: module, supervisor: supervisor, harvester: nil, timer: nil}}
  end
  ...
  defp trigger_harvest_cycle(%{name: name}),
    do: Process.send_after(self(), :harvest_cycle, NewRelic.Config.harvest_cycle(name))
end
